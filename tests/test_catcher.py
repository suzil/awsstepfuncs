...
def test_catcher():
    ...
    assert stdouts[failure_mock_fn] == [
        ...
        "Found catcher, transitioning to PassState(name='Pass', next_state='Failure')",
        "State output: {}",
        "Running PassState(name='Pass', next_state='Failure')",
        "State input: {}",
        'State input after applying input path of "$": {}',
        'Output from applying result path of "$": {}',
        'State output after applying output path of "$": {}',
