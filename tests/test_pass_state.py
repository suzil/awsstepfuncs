...
def test_pass_state(compile_state_machine):
    ...
    assert stdout == [
        ...
        "Running PassState(name='Pass 1', comment='The starting state', next_state='Pass 2')",
        "State input: {}",
        'State input after applying input path of "$": {}',
        'Output from applying result path of "$": {}',
        'State output after applying output path of "$": {}',
        "State output: {}",
        "Running PassState(name='Pass 2', next_state='Pass 3')",
        "State input: {}",
        'State input after applying input path of "$": {}',
        'Output from applying result path of "$": {}',
        'State output after applying output path of "$": {}',
        "State output: {}",
        "Running PassState(name='Pass 3')",
        "State input: {}",
        'State input after applying input path of "$": {}',
        'Output from applying result path of "$": {}',
        'State output after applying output path of "$": {}',
